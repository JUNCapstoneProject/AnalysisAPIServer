...
public final class NewsService {
    ...
    /**
     * 외부 HTTPS 요청을 받아 내부적으로 웹소켓 통신을 수행하는 메인 메소드입니다.
     *
    ...
    createErrorDto(final ErrorCode errorCode) {
        return AnalyzeResponseDto.builder()
                ...
                // ⭐️ 오류 수정: getStatusCode() -> getStatus().value()
    ...
    /**
     * 내부 stockanalysisAPI 와의 통신을 전담하는 핸들러 클래스입니다.
     */
    @Slf4j
    @RequiredArgsConstructor
    private static final class InternalApiHandler extends TextWebSocketHandler {
        /**
        ...
        private final ObjectMapper objectMapper;
        /**
         * 내부 API 서버로 전송할 뉴스 데이터 아이템입니다.
         */
        private final NewsItemDto itemToSend;
